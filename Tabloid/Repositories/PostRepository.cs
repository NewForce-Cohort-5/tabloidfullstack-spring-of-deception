...
namespace Tabloid.Repositories
{
    public class PostRepository : BaseRepository, IPostRepository
    {
        ...
        public List<Post> GetAllPostsByUserWithComments(int userProfileId)
        {
            using (var conn = Connection)
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                       SELECT p.Id AS PostId, p.Title, p.Content, 
                              ...
                              u.UserTypeId, 
                              ut.[Name] AS UserTypeName,
                              com.Id AS CommentId, com.Content AS Comment, com.CreateDateTime AS CommentDateTime,
                              up.DisplayName AS CommentName
                         FROM Post p
                              LEFT JOIN Category c ON p.CategoryId = c.id
                              LEFT JOIN UserProfile u ON p.UserProfileId = u.id
                              LEFT JOIN UserType ut ON u.UserTypeId = ut.id
                              LEFT JOIN Comment com ON com.PostId = p.id
                              LEFT JOIN UserProfile up ON com.UserProfileId = up.Id
                        WHERE p.UserProfileId = @userProfileId";
